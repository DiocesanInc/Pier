...
if (!function_exists('pier_setup')) :
    ...
    function pier_setup()
    {
		 ...
		 * If you're building a theme based on Pier, use a find and replace
		 * to change 'pier' to the name of your theme in all the template files.
		 */
        load_theme_textdomain('pier', get_template_directory() . '/languages');
        ...
        register_nav_menus(
            array(
                'menu-1' => esc_html__('Primary', 'pier'),
                'footer-quicklinks' => esc_html__('Footer Quicklinks', 'pier'),
            )
        );
...
add_action('after_setup_theme', 'pier_setup');
 ...
 */
function pier_content_width()
{
    $GLOBALS['content_width'] = apply_filters('pier_content_width', 640);
}
add_action('after_setup_theme', 'pier_content_width', 0);
 ...
 */
function pier_widgets_init()
{
    register_sidebar(
        array(
            'name'          => esc_html__('Sidebar', 'pier'),
            'id'            => 'sidebar-1',
            'description'   => esc_html__('Add widgets here.', 'pier'),
...
add_action('widgets_init', 'pier_widgets_init');
