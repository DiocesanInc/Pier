...
add_action('wp_enqueue_scripts', 'pierStyles');
...
if (!function_exists("includeCustomVariables")) {
    ...
    {
        if (class_exists("CssHelper") && file_exists(WP_PLUGIN_DIR . "/css-helper/pier-custom-variables.css")) {
            \CssHelper::enqueueCssFile("pier-custom-variables");
        } else {
...
function addEditorStyles()
{
    ...
    if (class_exists("CssHelper") && file_exists(WP_PLUGIN_DIR . "/css-helper/pier-custom-variables.css")) {
        add_editor_style("../../plugins/css-helper/pier-custom-variables.css");
    } else {
